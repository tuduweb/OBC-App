...
void RGBRender::initialize()
{
	...
	**/
	...
	const char* fsrc =
		"varying mediump vec3 textureOut;\n"
		"uniform sampler2D tex;\n"
		"void main(void)\n"
		"{\n"
		"gl_FragColor = texture2D(tex, textureOut.xy); \n"
		"}\n";
...
// yuy2
void RGBRender::render(uchar* rgb888Ptr, int w, int h)
{
    if (!rgb888Ptr)return;
	...
	program.setAttributeBuffer("texCoord", GL_FLOAT, 2 * 4 * sizeof(GLfloat), 2, 2 * sizeof(GLfloat));
	...
	//y0uy1v -> 行数/2 高度不变
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, w, h, 0, GL_RGB, GL_UNSIGNED_BYTE, rgb888Ptr);
	...
	program.setUniformValue("tex", idYUYV);
	//texl_w
